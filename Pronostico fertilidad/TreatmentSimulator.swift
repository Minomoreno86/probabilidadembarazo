...
class TreatmentSimulator {
    ...
    // MARK: - 🧪 DEMOSTRACIÓN DE LA NUEVA LÓGICA
    
    /// Función de ejemplo para demostrar la nueva lógica con datos específicos
    func demonstrateNewLogic() {
        print("🧪 DEMOSTRACIÓN DE LA NUEVA LÓGICA DE FACTORES MODIFICABLES Y NO MODIFICABLES")
        ...
        print("   • Edad: 22 años")
        print("   • AMH: 0.5 ng/mL (NO MODIFICABLE)")
        print("   • TSH: 7.0 mIU/L (MODIFICABLE)")
        print("   • Mioma: Intramural 4cm (MODIFICABLE)")
        print("   • BMI: 22.0 (Normal)")
        
        // Análisis completo
        let completeAnalysis = generateCompleteFactorAnalysis(profile: profile)
        
        print("\n📋 RESUMEN DEL ANÁLISIS:")
        print(completeAnalysis.summary)
        
        // Factores modificables
        print("\n🔧 FACTORES MODIFICABLES:")
        if completeAnalysis.modifiableFactors.isEmpty {
            print("   • No se identificaron factores modificables")
        } else {
            for factor in completeAnalysis.modifiableFactors {
                print("   • \(factor.factor): +\(String(format: "%.1f", factor.improvement))%")
                print("     Recomendación: \(factor.recommendation)")
            }
        }
        
        // Factores no modificables
        print("\n⚠️ FACTORES NO MODIFICABLES:")
        if completeAnalysis.nonModifiableFactors.isEmpty {
            print("   • No se identificaron factores no modificables")
        } else {
            for factor in completeAnalysis.nonModifiableFactors {
                print("   • \(factor.factor) (\(factor.currentValue))")
                print("     Severidad: \(factor.severity)")
                print("     Impacto: \(factor.impact)")
                print("     Explicación: \(factor.explanation)")
                print("     Implicación clínica: \(factor.clinicalImplication)")
            }
        }
        
        // Recomendación de tratamiento
        print("\n🎯 RECOMENDACIÓN DE TRATAMIENTO:")
        print("   • Plan: \(completeAnalysis.treatmentRecommendation.plan)")
        print("   • Razonamiento: \(completeAnalysis.treatmentRecommendation.rationale.joined(separator: ", "))")
        
        print("\n" + String(repeating: "=", count: 60))
        print("✅ LÓGICA COMPLETA: Factores modificables + no modificables")
        print("🎯 UX MEJORADO: Clasificación clara de limitaciones")
    }
    
    // MARK: - 🧪 PRUEBA ESPECÍFICA PARA DATOS DEL USUARIO
    
    /// Función de prueba para datos específicos: 30 años, TSH 7, AMH 1.5
    func testUserData() {
        print("🧪 PRUEBA CON DATOS ESPECÍFICOS DEL USUARIO")
        print("=" + String(repeating: "=", count: 60))
        
        // Datos específicos del usuario
        let profile = FertilityProfile()
        profile.age = 30.0
        profile.tshValue = 7.0
        profile.amhValue = 1.5
        profile.bmi = 22.0  // Normal
        profile.myomaType = .none
        profile.polypType = .none
        profile.endometriosisStage = 0
        profile.adenomyosisType = .none
        profile.hsgResult = .normal
        ...
        print("\n" + String(repeating: "=", count: 60))
        print("✅ PRUEBA COMPLETADA: Verificar resultados en la app")
    }
